...
export async function createCity(newName) {
    try {
        const cookieStore = await cookies();
        const token = cookieStore.get("token")?.value;

        const response = await fetch("https://optima.trio-verse.com/api/v1/city", {
            method: "POST",
            headers: {
...
export async function updateCity(id, newName) {
    try {
        const cookieStore = await cookies();
        const token = cookieStore.get("token")?.value;

        const response = await fetch(`https://optima.trio-verse.com/api/v1/city/${id}`, {
            method: "PATCH",
            headers: {
                "Content-Type": "application/json",
                Authorization: `Bearer ${token}`
