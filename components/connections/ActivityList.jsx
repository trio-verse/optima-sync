...
export default function ActivityList({ connectionId, orgId, clientId, connectionName }) {
    ...
    return (
        <div className="mt-3 pt-3 border-t border-slate-100">
            {toast && (
                <div
                    className={`fixed top-5 left-1/2 -translate-x-1/2 z-[70] px-6 py-3 rounded-xl shadow-lg text-sm font-bold animate-in fade-in slide-in-from-top-2 ${
                ...
                <div className="space-y-2 max-h-60 overflow-y-auto pr-1">
                    {activities.map((activity, index) => (
                        <div
                            ...
                            className="group relative bg-slate-50 rounded-xl p-3 border border-slate-100 hover:border-slate-200 transition-all"
                        >
                            <div className="flex items-start gap-3">
                                ...
                                <div className="flex-1 min-w-0">
                                    <p className="text-sm text-slate-700 leading-relaxed break-words pr-12">
                                        {activity.content}
                                    </p>
                                    <div className="flex items-center gap-3 mt-1.5 text-xs text-slate-400">
                                        <span className="flex items-center gap-1">
                                            <User className="w-3 h-3" />
                                            {/* هنا الإصلاح: القراءة من user.name أو user */}
                            ...
                            {/* أزرار التعديل والحذف تظهر عند إشارة الماوس (Hover) */}
                            <div className="absolute top-2 right-2 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                <button
                                    onClick={() => handleOpenEdit(activity)}
                                    className="p-1 text-slate-400 hover:text-blue-600 rounded-md hover:bg-white transition-all"
                                    title="Edit"
                                >
                                ...
                                <button
                                    ...
                                    className="p-1 text-slate-400 hover:text-red-600 rounded-md hover:bg-white transition-all"
                                    title="Delete"
                                >
